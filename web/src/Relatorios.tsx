...
export default function Relatorios() {
  ...
  const [selTurnos, setSelTurnos] = useState<Set<string>>(new Set(["1","2","3"]));
  ...
  const toggleTurno = (id: string) => setSelTurnos(prev => { const s=new Set(prev); s.has(id)?s.delete(id):s.add(id); return s; });
  ...
  const [gTurnos, setGTurnos] = useState<Set<string>>(new Set(["1","2","3"]));
  ...
  /* -------- NOVA SEÇÃO: Paradas por Motivo (1 tear) -------- */
  const [mDtIni, setMDtIni] = useState<string>(() => dtIni);
  const [mDtFim, setMDtFim] = useState<string>(() => dtFim);
  const [mTurnos, setMTurnos] = useState<Set<string>>(new Set(["1","2","3"]));
  const [mTear, setMTear] = useState<number | null>(null);

  // novo: motivos selecionados (checkboxes)
  const [selMotivos, setSelMotivos] = useState<Set<number>>(new Set());
  useEffect(() => {
    // sempre que carregar/alterar a lista de motivos, marca todos
    setSelMotivos(new Set(motivos.map(m => m.codigo)));
  }, [motivos]);
    ...
    <div className="container-fluid py-3">
      <div className="row g-3">
          ...
          <div className="card shadow-sm">
            <div className="card-body">
              <div className="d-flex align-items-center justify-content-between mb-3">
                <h2 className="h3 m-0">Relatórios de Paradas e Funcionamento</h2>
              ...
              <div className="row g-3">
                ...
                <div className="col-12 col-md-3">
                  ...
                  <div className="d-flex gap-3 pt-2">
                    {["1","2","3"].map(t=>(
                      <label key={`turno-${t}`} className="form-check">
                        <input className="form-check-input" type="checkbox" checked={selTurnos.has(t)} onChange={()=>setSelTurnos(p=>{const s=new Set(p); s.has(t)?s.delete(t):s.add(t); return s;})} />
                  ...
                  <div className="d-flex flex-wrap gap-3">
                    <label className="form-check">
                      <input className="form-check-input" type="checkbox"
                        ...
                        onChange={() => setSelTeares(prev => prev.size===teares.length ? new Set() : new Set(teares.map(x=>Number(x.codigo))))}/>
                    ...
                    {teares.map(t=>(
                      <label key={`tear-${t.codigo}`} className="form-check">
                        <input className="form-check-input" type="checkbox"
                          ...
                          onChange={()=>setSelTeares(p=>{const s=new Set(p); s.has(Number(t.codigo))?s.delete(Number(t.codigo)):s.add(Number(t.codigo)); return s;})}/>
                        <span className="form-check-label">{t.nome}</span>
                      </label>
                    ))}
                  </div>
                  <div className="text-muted small mt-2">
                    Usa <code>/turnos</code> por dia e <code>/eventos</code> para horas paradas.
                  </div>
                </div>
              </div>
          ...
          <div className="card shadow-sm">
            <div className="card-body">
              <h3 className="h5 mb-3">
                Controle de Paradas — Gráfico {gTearName ? `(${gTearName})` : "(1 tear)"}
              </h3>
              ...
              <div className="row g-3">
                ...
                <div className="col-12 col-md-3">
                  ...
                  <div className="d-flex gap-3 pt-2">
                    {["1","2","3"].map(t=>(
                      <label key={`gturno-${t}`} className="form-check">
                        <input className="form-check-input" type="checkbox" checked={gTurnos.has(t)} onChange={()=>{
                          setGTurnos(p=>{const s=new Set(p); s.has(t)?s.delete(t):s.add(t); return s;});
                        }} />
                        <span className="form-check-label">{t}</span>
                      </label>
                    ))}
        ...
        <div className="col-12">
          <div className="card shadow-sm">
            <div className="card-body">
              <h3 className="h5 mb-3">Paradas por Motivo — {mTearName ? `(${mTearName})` : "(1 tear)"}</h3>
              ...
              <div className="row g-3">
                ...
                <div className="col-12 col-md-3">
                  ...
                  <div className="d-flex gap-3 pt-2">
                    {["1","2","3"].map(t=>(
                      <label key={`mturno-${t}`} className="form-check">
                        <input className="form-check-input" type="checkbox" checked={mTurnos.has(t)} onChange={()=>{
                          setMTurnos(p=>{const s=new Set(p); s.has(t)?s.delete(t):s.add(t); return s;});
                        }} />
                        <span className="form-check-label">{t}</span>
                      </label>
                    ))}
